...
def inference_seg(i):
    ...
    label_flip_dims=[3,2]
    subject_id = i
    subject_name = 'seg/%d/' % subject_id

    f_T1 = os.path.join(test_path, subject_name + 'T1.nii.gz')
    f_T2 = os.path.join(test_path, subject_name + 'T2.nii.gz')
    f_l = os.path.join(test_path, subject_name + 'cp_label.nii.gz')
    #f_T1 = os.path.join(test_path, 'T1.nii.gz')
    #f_l = os.path.join(test_path, 'label.nii.gz')
    inputs_T1, img_T1_itk = read_med_image(f_T1, dtype=np.float32)
    inputs_T2, img_T2_itk = read_med_image(f_T2, dtype=np.float32)
    ...
    whole_pred.SetOrigin(origin)
    whole_pred.SetDirection(direction)
    path = os.path.join(outdir,  'sub%d_T1.nii.gz'%subject_id)
    sitk.WriteImage(whole_pred,path)
    print(subject_id,'done')
